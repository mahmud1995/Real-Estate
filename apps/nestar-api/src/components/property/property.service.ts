...
export class PropertyService {
	...
	public async getProperty(memberId: ObjectId, propertyId: ObjectId): Promise<Property> {
		const search: T = {
			_id: propertyId,
			propertyStatus: PropertyStatus.ACTIVE, // find me an active property with this ID
		};

		const targetProperty: Property = await this.propertyModel.findOne(search).lean<Property>().exec();
		// Instead of returning a heavy Mongoose document, it returns a plain JS object as Property
	...
	public async updateProperty(memberId: ObjectId, input: PropertyUpdate): Promise<Property> {
		...
		const search: T = {
			_id: input._id,
			memberId: memberId, // faqat uzinikini yangilay olsin
				...
				{ $sort: sort },
				{
					$facet: {
						list: [
							...
							{ $unwind: '$memberData' }, //memberData qiymatiga tenglab beradi
						],
						metaCounter: [{ $count: 'total' }],
					},
				},
